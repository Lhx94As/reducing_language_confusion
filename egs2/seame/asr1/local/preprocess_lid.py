...
def remove_control_chars(text):
    ...
    return control_char_re.sub("", text) #control characters are: \n, \t, \r, etc.


def remove_redundant_whitespaces(text):
    """remove redundant whitespaces"""
    return re.sub(" +", " ", text).strip() #Don't know why but multiple spaces are replaced by single one
...
def remove_repeated_noise(text, pattern="<noise>"):
    ...
    if len(re.findall(pattern, text)) <= 1: #find all patterns in text
        return text
...
def normalize_text(text):
    ...
    return output_text # seems that the output_text should only contain common characters, <noise>, and <unk>
...
def write_mandarin_only_text(data_dict, file, char_file1, char_file2):
    ...
    # counter = {"hi":33, "ni":16, ..}
    # sorted counter.keys() generates unique Mandarin characters
...
# language identification part

def transform_text_to_lang(text_list):
    new_text = []
    for x in text_list:
        try:
            x_norm = normalize_text(x.strip().strip("▁")[0])
            if is_english(x_norm):
                new_text.append("e")
            elif is_mandarin(x_norm):
                new_text.append("m")
            elif x_norm.isdigit() or x_norm == "'":
                new_text.append("d")
            else:
                new_text.append(x)
        except:
...
def transform_text_to_lang2(text_list):
    new_text = []
    for x in text_list:
        try:
            x_norm = normalize_text(x.strip().strip("▁")[0])
...
def remove_extra_english_labels(labels):
    start = None
    output_labels = ""
    new_labels = []
    for i in labels:
        if i != start:
            ...
            pass
    return output_labels.join(new_labels)
...
if __name__ == "__main__":
    ...
    for atp in audio_type:
        ...
        for au in os.listdir(os.path.join(ori_data_pth, atp, "audio")):
            audios.append(au.strip(".flac")) #audio names with abs paths
            audio_idx_list.append(au.split("/")[-1].strip(".flac").lower()) #audio names without abs paths
