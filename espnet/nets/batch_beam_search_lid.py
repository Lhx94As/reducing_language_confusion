...
from espnet.nets.beam_search_lid import BeamSearch, Hypothesis
...
class BatchBeamSearch(BeamSearch):
    ...
    def token_to_language(self, hyp):
        token_seq = hyp.yseq
        language_seq = torch.zeros_like(token_seq)
        for batch in range(token_seq.size(0)):
            for tki in range(token_seq.size(1)):
                token_ = self.token_list[token_seq[batch, tki]]
                lid_token = self.token_to_lid(token_)
                language_seq[batch, tki] = self.token_id_converter_lid.tokens2ids(lid_token)[0]
        return language_seq
    ...
    def score_full(
        ...
        scores = dict()
        states = dict()
        # lid_mask = subsequent_mask(hyp.yseq.size(-1), device=x.device).unsqueeze(0)
        x_len = torch.LongTensor([x.size(1)]).repeat(x.size(0)).type_as(x)
        sos_lid = torch.zeros((hyp.yseq.size(0),1,4))
        sos_lid[:,:,-1]=1
        ys_in_len = torch.LongTensor([hyp.yseq.size(1)]).repeat(hyp.yseq.size(0)).type_as(x)
        lid_post, _ = self.decoder_lid(x, x_len, hyp.yseq, ys_in_len)
        lid_post = torch.nn.functional.softmax(lid_post, dim=-1)
        lid_post = torch.concat((sos_lid.type_as(x), lid_post[:,:-1,:]), dim=1)
        # logging.info(f"lid post:{lid_post}")

        for k, d in self.full_scorers.items():
            if k != "decoder":
                scores[k], states[k] = d.batch_score(hyp.yseq, hyp.states[k], x)
            else:
                scores[k], states[k] = d.batch_score(hyp.yseq, hyp.states[k], x, lid_post)
