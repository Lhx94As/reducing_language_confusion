...
token_lid_list="${bpedir}"/tokens_lid.txt
if ${use_word_lm}; then
...
if ! "${skip_train}"; then
    ...
    if [ ${stage} -le 10 ] && [ ${stop_stage} -ge 10 ]; then
        ...
        # _asr_train_dir = dump/raw/train_sp/, _feat_type = raw, audio_format = flac
        ...
        # shellcheck disable=SC2046,SC2086
        ${train_cmd} JOB=1:"${_nj}" "${_logdir}"/stats.JOB.log \
            ${python} -m espnet2.bin.asr_lid_concat_train \
                --collect_stats true \
                --use_preprocessor true \
                --bpemodel "${bpemodel}" \
                ...
                --train_data_path_and_name_and_type "${_asr_train_dir}/text,lid_text,text" \
                --valid_data_path_and_name_and_type "${_asr_valid_dir}/${_scp},speech,${_type}" \
                --valid_data_path_and_name_and_type "${_asr_valid_dir}/text,text,text" \
                --valid_data_path_and_name_and_type "${_asr_valid_dir}/text,lid_text,text" \
                --train_shape_file "${_logdir}/train.JOB.scp" \
    ...
    if [ ${stage} -le 11 ] && [ ${stop_stage} -ge 11 ]; then
        ...
        if [ "${num_splits_asr}" -gt 1 ]; then
            ...
            _opts+="--train_data_path_and_name_and_type ${_split_dir}/text,lid_text,text "
            ...
            _opts+="--train_data_path_and_name_and_type ${_asr_train_dir}/text,lid_text,text "
    ...
    if [ ${stage} -le 12 ] && [ ${stop_stage} -ge 12 ]; then
        ...
        if "${use_k2}"; then
          ...
          if "${use_streaming}"; then
              ...
              asr_inference_tool="espnet2.bin.asr_lid_concat_inference"
          fi
        fi
